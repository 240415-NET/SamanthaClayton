...
{
    public static void CreateInitialSQLTables()
    {
        CreateInitialTables();
        CreateRecipes();
    }
    ...
    public static void CreateInitialTables()
    {
        ...
        // After creating the SqlConnection object, call the Open() method off of it to open the connection to the database
        mySQLConnection.Open();

        // Create a string for the SQL query or statement we are going to run
        string myCommandText =
            ...
            RecipeId UNIQUEIDENTIFIER PRIMARY KEY,
            RecipeName NVARCHAR(255) NOT NULL
            );
            ...
            RecipeId UNIQUEIDENTIFIER,
            RecipeStepId UNIQUEIDENTIFIER,
            RecipeStepSequence int,
            RecipeStepDescription NVARCHAR(255),
            PRIMARY KEY (RecipeId, RecipeStepId)
            );
            ";
            
        // Create a SqlCommand object that takes in the SQL code in the command text and the SqlConnection object
        using SqlCommand myCommand = new SqlCommand(myCommandText, mySQLConnection);

        // Once we create the SqlCommand object, we can run the ExecuteNonQuery() method off of it in order to
        // run execute the SQL code in the command text
        // Because we are not querying the DB (i.e., using a select statement), this is a nonquery
        myCommand.ExecuteNonQuery();

        // Close the connection (& because we used the 'using' statement above, the SqlConnection object will be disposed of)
        ...
        public static void AddNewRecipes(Recipes recipeToAdd)
        {
            ...
            // Create the SqlConnection object & provide it with the connection string
            using SqlConnection mySQLConnection = new SqlConnection(connectionString);

            // Once the object is created, call the Open() method off of it in order to open
            // the connection to the database
            mySQLConnection.Open();

            // Create a string with the SQL code that will execute
            // This code allows us to update the parameters later
            string myCommandTextForRecipeNamesTable =

                 @"INSERT INTO Recipe_Names (RecipeId, RecipeName)
                 VALUES (@RecipeId, @RecipeName);";

            // Create a SqlCommand object & pass it both the SQL code we want to execute and the SqlConnection object
            ...
            // Execute the nonquery because we aren't querying the database (i.e., we're not using a select statement)
            myCommandForRecipeNamesTable.ExecuteNonQuery();

            // For eaach ingredient in a recipe, provide the ingredient's info and insert it into the SQL table
            // Then move on to the next ingredient in the recipe
            ...
            // For each step in a recipe, provide the step's info and insert it into the SQL table
            ...
            // Close the SQL connection (& then the SQLConnection object will be disposed of because we
            // created it with the 'using' statement above.
            mySQLConnection.Close();
